// Sección 11 — Fotovoltaica por central (barras horizontales)
import { useEffect, useRef } from 'react'
import * as d3 from 'd3'

export default function SolarFarmBars({ data = [] }) {
  ...
  useEffect(() => {
    ...
    const rowH   = 22
    const margin = { top: 8, right: 95, bottom: 28, left: 155 }
    ...
    const y = d3.scaleBand().domain(sorted.map(d => d.central)).range([0, H - margin.top - margin.bottom]).padding(0.15)
    ...
    g.selectAll('.val')
      ...
      .attr('x', d => x(d.valor) + 5).attr('y', d => y(d.central) + y.bandwidth() / 2)
      .attr('dy', '0.35em').attr('fill', '#F59E0B')
